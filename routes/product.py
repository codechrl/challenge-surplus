...
from schema.message import IdResponse, ProductResponse, Response
from util.validation import is_empty

router = APIRouter()


@router.get("", status_code=status.HTTP_200_OK, response_model=ProductResponse)
async def get_product(
    ...
    try:
        ...
        table = Table("product", db.get_metadata(), autoload_with=db.get_engine())
...
async def post_product(payload: schema.ProductPost):
    try:
        database = db.get_database()
        table = Table("product", db.get_metadata(), autoload_with=db.get_engine())
...
async def put_product(id: int, payload: schema.ProductPut):

    if await is_empty(table="product", conditions={"id": id}):
